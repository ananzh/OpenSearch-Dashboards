...
import { useDispatch, useSelector, setUIStateState } from '../utils/state_management';
...
export const WorkspaceUI = () => {
  ...
  useEffect(() => {
    if (rootState.vbEditor.status === 'loaded') {
      uiState.setSilent(rootState.vbUi);
    }
    // To update uiState once saved object data is loaded
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rootState.vbEditor.status, uiState]);
  ...
  useEffect(() => {
    async function loadExpression() {
      ...
      const schemaValidation = validateSchemaState(schemas, rootState.vbVisualization);

...
export const LogActionMenu: React.FC<LogActionMenuProps> = ({
  ...
  // Debug logging
  console.log('🎯 [LogActionMenu] Actions check:', {
    document: !!document,
    indexPattern,
    compatibleActionsCount: compatibleActions.length,
    compatibleActions: compatibleActions.map(action => ({
      id: action.id,
      displayName: action.displayName,
      iconType: action.iconType
    })),
    allRegisteredActions: logActionRegistry.getAllActions()
  });
  ...
  const handleActionResult = useCallback((result: { success: boolean; message?: string }) => {
    ...
    if (result.success) {
      console.log('✅ [LogActionMenu] Action completed:', result.message);
      // Could show success toast here
    } else {
      console.log('❌ [LogActionMenu] Action failed:', result.message);
  ...
  if (compatibleActions.length === 0) {
    console.log('❌ [LogActionMenu] No compatible actions found - returning null');
    return null;
  }

  console.log('✅ [LogActionMenu] Rendering menu with', compatibleActions.length, 'compatible actions');
  ...
  return (
    ...
    >
      {selectedAction ? (
        ...
        <div>
          <EuiFlexGroup alignItems="center" gutterSize="s" style={{ padding: '8px 16px', borderBottom: '1px solid #D3DAE6' }}>

...
export class CoActAgent implements BaseAgent {
  ...
  private buildStateGraph(): void {
    ...
    const CoActStateAnnotation = Annotation.Root({
      ...
      todoList: Annotation<AgentTodoList>({
        reducer: (x: AgentTodoList, y: AgentTodoList) => y || x,
        default: () => ({
          todos: [],
          currentIndex: 0,
          executionOrder: [],
      ...
      executionContext: Annotation<Map<string, any>>({
        reducer: (x: Map<string, any>, y: Map<string, any>) => y || x,
      ...
      lastToolExecution: Annotation<number | undefined>({
    ...
    this.graph = new StateGraph(CoActStateAnnotation);

    // Add nodes (avoiding reserved names)
    // Cast to any to work around TypeScript strict typing with Annotation API
    this.graph.addNode('processInput', this.processInputNode.bind(this) as any);
    this.graph.addNode('callModel', this.callModelNode.bind(this) as any);
    this.graph.addNode('executeTools', this.executeToolsNode.bind(this) as any);
    this.graph.addNode('generateResponse', this.generateResponseNode.bind(this) as any);
    ...
    this.graph.addConditionalEdges('callModel' as any, (state: CoActAgentState) => {
    ...
    this.graph.addConditionalEdges('executeTools' as '__start__', (state: CoActAgentState) => {
...
Remember: Output ONLY the JSON object, no additional text.`;
  ...
  private async verificationNode(state: CoActAgentState): Promise<Partial<CoActAgentState>> {
    ...
    const completeness =
      todoList.todos.filter((t) => t.status === 'completed').length / todoList.todos.length;
    ...
    const qualityScore =
      completeness * 0.3 + accuracy * 0.3 + relevance * 0.2 + clarity * 0.1 + actionability * 0.1;
